document.addEventListener('DOMContentLoaded', function () {
    ...
    let restauranteEmail = '';
    let restauranteNombre = '';

    // Escuchar el cambio de selección en el dropdown de restaurante
    selectRestaurante.addEventListener("change", function () {
        restauranteEmail = this.value; // Obtiene el correo del restaurante seleccionado
        restauranteNombre = this.options[this.selectedIndex].text; // Obtiene el nombre del restaurante seleccionado
        console.log("Correo del restaurante seleccionado:", restauranteEmail);
        console.log("Nombre del restaurante seleccionado:", restauranteNombre);
    });
    ...
    document.getElementById('reservaForm').addEventListener('submit', function (event) {
        ...
        if (!confirm('¿Confirmas tu reserva en ' + restauranteNombre + '?')) {
            return;
        }
        ...
        const mensajeUsuario = `
            Nueva reserva en ${restauranteNombre}:
            - Nombre: ${nombre}
            - Correo: ${email}
            - Teléfono: ${telefono}
        ...
        const mensajeRestaurante = `
            Nueva reserva recibida en ${restauranteNombre}:
        ...
        emailjs.send('service_32kp1xh', 'template_ezkb5p1', {
            to_email: email,
            from_name: nombre,
